...
(function ($) {
    ...
    KEditor.DEFAULTS = {
        ...
        tabContainersText: 'Layouts',
        tabContainersTitle: 'Layouts',
        tabComponentsText: 'Media',
        tabComponentsTitle: 'Media',
        tabTooltipEnabled: true,
        extraTabs: null,
        defaultComponentType: 'blank',
        sidebarContainer: null,
        snippetsUrl: plugin_url + 'snippets/default/snippets.php',
        ...
        if (typeof options.snippetsUrl === 'string' && options.snippetsUrl.length > 0) {
            ...
            return $.ajax({
                ...
                success: function (resp) {
					...
					var str = resp.replace(new RegExp('snippets/', 'g'),plugin_url + 'snippets/'); 
                    self.renderSnippets(str);
                    self.initSnippets();
                    self.initTabs();
    ...
    KEditor.prototype.renderSnippets = function (resp) {
        ...
        $.get( plugin_url + 'includes/attachments.php', function( data ) {
			console.log(data);
		});
