    ...
    KEditor.components['photo'] = {
        ...
        initSettingForm: function (form, keditor) {
            ...
            photoEdit.on('click', function (e) {
                // Andrew was Here.
                // little tweak due to deadline
                // need to clean up here. -- later
                e.preventDefault();
                $("#window").fadeIn(200);
                $(document).on('click',"#chooseImage", function(e){
                    ...
                    $imageinfo = $("#imageInfo");
                    img.attr('src',$imageinfo.find('img').first().attr('src'));
                    img.attr('title',$imageinfo.find('input[name="title"]').first().val());
                    img.attr('alt',$imageinfo.find('input[name="image_alt"]').first().val());
                    //img.attr('description',$imageinfo.find('input[name="title"]').first().val());
                    $("#window").fadeOut(200);
                });
                //fileInput.trigger('click');
            });
...
(function ($) {
    ...
    KEditor.components['text'] = {
        options: {
            ...
            title: false,
            allowedContent: true, // D ISABLES Advanced Content Filter. This is so templates with classes: allowed through
            bodyId: 'editor',
        ...
        initSettingForm: function (form, keditor) {
            ...
            form.append(
                ...
                */
                ...
                '<div class="col-sm-12">' +
                '<div class="video-toolbar">' +
                '<a href="#" class="btn-videoFileInput btn btn-sm btn-primary"><i class="fa fa-upload"></i> Choose Video</a>' +
                '<input id="videoFileInput" type="file" style="display: none">' +
                '</div>' +
                '</div>' +
                '</div>' +
                ...
                '<div class="form-group">' +
                '<label for="video-width" class="col-sm-12">Width (%)</label>' +
                '<div class="col-sm-12">' +
                '<input type="number" id="video-width" min="0" max="100" class="form-control" value="100" />' +
                '</div>' +
                '</div>' +
                '</form>'
            );
        ...
        showSettingForm: function (form, component, keditor) {
            ...
            btnVideoFileInput.on('click', function (e) {
                ...
                $(document).on('click',"#chooseImage", function(e){
                    ...
                    $imageinfo = $("#imageInfo");
                    $("#window").fadeOut(200);
                    video.attr('src',$imageinfo.find('source').first().attr('src'));
